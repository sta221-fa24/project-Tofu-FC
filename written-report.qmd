...
mushrooms <- mushrooms |>
  mutate(
    ...
    gill.color.modified = case_when(
      gill.color %in% c("w", "y", "n", "p") ~ gill.color, TRUE ~ "Other"),
...
Our final model is a logistic regression model predicting the binary response variable `class_binary` as a function of the following predictors: `cap.diameter`, `season`, `cap.shape`, `cap.color`, `gill.color`, `stem.color.modified`, `habitat.modified`, `habitat.modified*season`, and `cap.shape*cap.color`. Full model coefficients can be seen in the appendix (Table 11).
...
plt1 + plt2
```
The final model was evaluated with the testing data. The model is decent as the AUC is 0.797 which is closer to 1 than 0.5. We decided on a threshold of p = 0.247 to achieve a sensitivity of 95%, since we wanted to prioritize minimizing false negatives, which are more expensive – better to be careful than eat a poisonous mushroom classified as "edible".
